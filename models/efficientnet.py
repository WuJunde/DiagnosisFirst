...
class EfficientNet(nn.Module):
    ...
    def __init__(self, device , type, blocks_args=None, global_params=None):
        super().__init__()
        ...
        # Build blocks freeze
        self._blocks_freeze = nn.ModuleList([])
        index = 0
        for block_args in self._blocks_args:
            ...
            # The first block needs to take care of stride and filter size increase.
            self._blocks_freeze.append(MBConvBlock_freeze(block_args,index, device, self._global_params))
            index += 1
            if block_args.num_repeat > 1:
                block_args = block_args._replace(input_filters=block_args.output_filters, stride=1)
            for _ in range(block_args.num_repeat - 1):
    ...
    def extract_features_freeze(self, inputs,weights):
        """ Returns output of the final convolution layer """

        # Stem
        x = F.conv2d(inputs, weights['_conv_stem.weight'], stride=2)
        x = F.batch_norm(x, torch.zeros(x.data.size()[1]).to(self.device),
                         torch.ones(x.data.size()[1]).to(self.device),
                         weights['_bn0.weight'],
                         weights['_bn0.bias'],
        ...
        # Blocks
        for idx, block_freeze in enumerate(self._blocks_freeze):
            drop_connect_rate = self._global_params.drop_connect_rate
            if drop_connect_rate:
                drop_connect_rate *= float(idx) / len(self._blocks)
            x = block_freeze(x, weights, drop_connect_rate=drop_connect_rate)

        # Head
        x = F.conv2d(x, weights['_conv_head.weight'])
        x = F.batch_norm(x, torch.zeros(x.data.size()[1]).to(self.device),
                         torch.ones(x.data.size()[1]).to(self.device),
                         weights['_bn1.weight'],
                         weights['_bn1.bias'],
    ...
    def forward(self, inputs, weights=None):
        ...
        if weights is None:
            ...
            bs = inputs.size(0)
            x = self.extract_features_freeze(inputs,weights)
            if self.type == 'map':
                reg = F.conv2d(x, weights['conv_reg.weight'],
                               weights['conv_reg.bias'])  # reg
                reg = self.sig(reg)
            elif self.type == 'big_map':
                reg = F.upsample(x,scale_factor=2, mode='bilinear')
                reg = F.relu(reg)
                reg = F.conv2d(reg, weights['conv_transe1.weight'],
                               weights['conv_transe1.bias'])
                reg = F.batch_norm(reg, torch.zeros(reg.data.size()[1]).to(self.device),
                                 torch.ones(reg.data.size()[1]).to(self.device),
                                 weights['bn_transe1.weight'],
                                 weights['bn_transe1.weight'],
                                 training=True)

                reg = F.upsample(reg,scale_factor=2, mode='bilinear')
                reg = F.relu(reg)
                reg = F.conv2d(reg, weights['conv_transe2.weight'],
                               weights['conv_transe2.bias'])
                reg = F.batch_norm(reg, torch.zeros(reg.data.size()[1]).to(self.device),
                                 torch.ones(reg.data.size()[1]).to(self.device),
                                 weights['bn_transe2.weight'],
                                 weights['bn_transe2.weight'],
                                 training=True)

                reg = F.conv2d(reg, weights['conv_transe_mask.weight'],
                               weights['conv_transe_mask.bias'])
                reg = self.sig(reg)
                ...
                reg = F.conv2d(reg, weights['conv_transe3.weight'],
                               weights['conv_transe3.bias'])
                reg = self.sig(reg)
            ...
            x = x.view(bs, -1)
            x = self._dropout(x)
            x = F.linear(x,weights['_fc.weight'],weights['_fc.bias'])

        return x, reg

    @classmethod
    def from_name(cls, model_name, device, type, override_params=None):
        cls._check_model_name_is_valid(model_name)
        blocks_args, global_params = get_model_params(model_name, override_params)
        return cls(device, type, blocks_args,  global_params)
